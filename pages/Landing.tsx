...
const FAQItem: React.FC<{ q: string; a: string }> = ({ q, a }) => {
  ...
  return (
    <div style={{ border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden', marginBottom: 12 }}>
      <button
        onClick={() => setOpen(o => !o)}
        style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 24px', background: open ? 'rgba(124,92,252,0.08)' : 'rgba(19,21,30,0.8)', textAlign: 'left', cursor: 'pointer', border: 'none', color: '#E8EAF0', transition: 'background 0.2s' }}
      >
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0C0E14', color: '#E8EAF0', fontFamily: 'DM Sans, sans-serif', overflowX: 'hidden' }}>
      ...
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', overflow: 'hidden', zIndex: 0 }}>
        <div style={{ position: 'absolute', top: '-10%', left: '5%', width: 700, height: 700, background: 'rgba(124,92,252,0.07)', borderRadius: '50%', filter: 'blur(120px)' }} />
        <div style={{ position: 'absolute', top: '40%', right: '-10%', width: 500, height: 500, background: 'rgba(139,92,246,0.05)', borderRadius: '50%', filter: 'blur(100px)' }} />
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(12,14,20,0.85)', backdropFilter: 'blur(20px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 12, background: 'linear-gradient(135deg, #7C5CFC, #A78BFA)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(124,92,252,0.35)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button onClick={() => setAuthMode('register')} style={{ padding: '8px 20px', borderRadius: 10, background: 'linear-gradient(135deg, #7C5CFC, #9B7EFD)', border: 'none', color: 'white', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, boxShadow: '0 4px 16px rgba(124,92,252,0.3)', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <section style={{ position: 'relative', zIndex: 1, paddingTop: 140, paddingBottom: 100, textAlign: 'center', padding: '140px 24px 100px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          {/* Badge */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 999, background: 'rgba(124,92,252,0.1)', border: '1px solid rgba(124,92,252,0.25)', color: '#A78BFA', fontSize: 12, fontWeight: 600, marginBottom: 32, letterSpacing: '0.05em' }}>
          ...
          <h1 style={{ ...S, fontSize: 'clamp(44px, 7vw, 76px)', fontWeight: 800, lineHeight: 1.08, marginBottom: 24, letterSpacing: '-0.02em' }}>
            ...
            <br />
            <span style={{ background: 'linear-gradient(135deg, #7C5CFC, #C4B5FD)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Instantly.</span>
          </h1>
        ...
        <div style={{ maxWidth: 1000, margin: '72px auto 0', position: 'relative' }}>
          <div style={{ background: 'linear-gradient(135deg, rgba(124,92,252,0.15), rgba(124,92,252,0.05))', border: '1px solid rgba(124,92,252,0.2)', borderRadius: 24, padding: 3, boxShadow: '0 40px 100px rgba(0,0,0,0.5), 0 0 60px rgba(124,92,252,0.1)' }}>
              ...
              <div style={{ padding: '20px 24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <div style={{ padding: '6px 14px', borderRadius: 10, background: 'rgba(124,92,252,0.15)', border: '1px solid rgba(124,92,252,0.25)', color: '#A78BFA', fontSize: 12 }}>Advanced Filters</div>
                ...
                {[
                  ...
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '100px 1fr 120px 90px 80px', gap: 16, padding: '10px 12px', borderRadius: 10, background: i === 1 ? 'rgba(124,92,252,0.07)' : 'transparent', borderBottom: '1px solid rgba(255,255,255,0.03)', alignItems: 'center' }}>
                    <span style={{ fontSize: 12, color: '#9B7EFD', fontWeight: 600, fontFamily: 'monospace' }}>{row.mc}</span>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 6, background: row.entity === 'CARRIER' ? 'rgba(124,92,252,0.1)' : 'rgba(251,146,60,0.1)', color: row.entity === 'CARRIER' ? '#A78BFA' : '#FB923C', border: `1px solid ${row.entity === 'CARRIER' ? 'rgba(124,92,252,0.2)' : 'rgba(251,146,60,0.2)'}`, display: 'inline-block' }}>{row.entity}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Floating stat cards */}
          <div style={{ position: 'absolute', top: -16, right: -24, background: '#1A1C27', border: '1px solid rgba(124,92,252,0.2)', borderRadius: 16, padding: '14px 20px', boxShadow: '0 16px 40px rgba(0,0,0,0.4)', display: 'none' }} className="md:block">
            <div style={{ fontSize: 10, color: '#475569', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Email Coverage</div>
            <div style={{ ...S, fontSize: 24, fontWeight: 700, color: '#A78BFA' }}>68.4%</div>
          </div>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '80px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(420px, 1fr))', gap: 64, alignItems: 'center' }}>
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderRadius: 999, background: 'rgba(124,92,252,0.1)', border: '1px solid rgba(124,92,252,0.2)', color: '#9B7EFD', fontSize: 11, fontWeight: 700, marginBottom: 20, letterSpacing: '0.06em' }}>
              🔍 POWERFUL SEARCH
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {['Instant search by name, DOT, or MC number', 'Advanced filters — state, cargo type, hazmat, fleet size', 'Filter by insurance company and renewal dates', 'Find carriers with email contacts only'].map(item => (
                <div key={item} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'rgba(124,92,252,0.15)', border: '1px solid rgba(124,92,252,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div style={{ marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {['State: TX', 'Entity: Carrier', 'Has Email: Yes', 'Active: Yes'].map(f => (
                  <div key={f} style={{ padding: '8px 12px', background: 'rgba(124,92,252,0.08)', border: '1px solid rgba(124,92,252,0.2)', borderRadius: 10, fontSize: 12, color: '#A78BFA', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Filter size={11} /> {f}
                  </div>
                ))}
              </div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(420px, 1fr))', gap: 64, alignItems: 'center' }}>
          ...
          <div style={{ background: '#13151E', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 24, padding: 28, boxShadow: '0 24px 60px rgba(0,0,0,0.4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24, paddingBottom: 20, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ width: 52, height: 52, borderRadius: 16, background: 'linear-gradient(135deg, #7C5CFC, #A78BFA)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 24px rgba(124,92,252,0.35)' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 10, padding: '2px 10px', borderRadius: 6, background: 'rgba(124,92,252,0.1)', color: '#A78BFA', border: '1px solid rgba(124,92,252,0.2)', fontWeight: 700 }}>CARRIER</span>
                </div>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
              {[{ l: 'DOT #', v: '3821944', c: '#9B7EFD' }, { l: 'MC #', v: '123456', c: '#9B7EFD' }, { l: 'Fleet Size', v: '47 units', c: 'white' }].map(s => (
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '80px 24px 120px' }}>
        <div style={{ maxWidth: 780, margin: '0 auto', textAlign: 'center', background: 'linear-gradient(135deg, rgba(124,92,252,0.12), rgba(139,92,246,0.06))', border: '1px solid rgba(124,92,252,0.2)', borderRadius: 32, padding: '72px 48px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: -60, right: -60, width: 300, height: 300, background: 'rgba(124,92,252,0.12)', borderRadius: '50%', filter: 'blur(60px)', pointerEvents: 'none' }} />
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 14px', borderRadius: 999, background: 'rgba(124,92,252,0.15)', border: '1px solid rgba(124,92,252,0.3)', color: '#A78BFA', fontSize: 12, fontWeight: 700, marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 28 }}>
            <button onClick={() => setAuthMode('register')} style={{ padding: '16px 40px', borderRadius: 14, background: 'linear-gradient(135deg, #7C5CFC, #9B7EFD)', border: 'none', color: 'white', fontSize: 16, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 32px rgba(124,92,252,0.4)', fontFamily: 'DM Sans, sans-serif', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {authMode && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'rgba(8,10,16,0.85)', backdropFilter: 'blur(10px)' }}>
          <div style={{ background: '#13151E', border: '1px solid rgba(255,255,255,0.08)', width: '100%', maxWidth: 440, padding: 36, borderRadius: 24, boxShadow: '0 40px 100px rgba(0,0,0,0.6), 0 0 0 1px rgba(124,92,252,0.1)', position: 'relative' }}>
            ...
            <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <button type="submit" disabled={isLoading}
                style={{ width: '100%', padding: '14px', borderRadius: 12, background: isLoading ? 'rgba(124,92,252,0.5)' : 'linear-gradient(135deg, #7C5CFC, #9B7EFD)', border: 'none', color: 'white', fontSize: 15, fontWeight: 700, cursor: isLoading ? 'not-allowed' : 'pointer', boxShadow: '0 4px 16px rgba(124,92,252,0.3)', fontFamily: 'DM Sans, sans-serif', marginTop: 4 }}>
            ...
            <div style={{ marginTop: 20, textAlign: 'center', fontSize: 13, color: '#334155' }}>
              {authMode === 'login' ? (
                <>Don't have an account?{' '}
                  <button onClick={() => setAuthMode('register')} style={{ color: '#9B7EFD', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Sign up free</button>
                </>
              ) : (
                <>Already have an account?{' '}
                  <button onClick={() => setAuthMode('login')} style={{ color: '#9B7EFD', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Log in</button>
                </>
              )}
            </div>
          </div>
