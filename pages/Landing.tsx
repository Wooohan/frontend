...
const FAQItem: React.FC<{ q: string; a: string }> = ({ q, a }) => {
  ...
  return (
    <div style={{ border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden', marginBottom: 12 }}>
      <button
        onClick={() => setOpen(o => !o)}
        style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 24px', background: open ? 'rgba(16,185,129,0.08)' : 'rgba(15,20,25,0.8)', textAlign: 'left', cursor: 'pointer', border: 'none', color: '#E8EAF0', transition: 'background 0.2s' }}
      >
        <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 600, fontSize: 15 }}>{q}</span>
        <ChevronDown size={18} style={{ color: '#10B981', transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.3s', flexShrink: 0, marginLeft: 16 }} />
      </button>
      {open && (
        <div style={{ padding: '0 24px 20px', background: 'rgba(15,20,25,0.6)', color: '#94A3B8', fontSize: 14, lineHeight: 1.7 }}>
          {a}
        </div>
      )}
    </div>
...
export const Landing: React.FC<LandingProps> = ({ onLogin }) => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080A0F', color: '#E8EAF0', fontFamily: 'DM Sans, sans-serif', overflowX: 'hidden' }}>

      {/* Background orbs */}
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', overflow: 'hidden', zIndex: 0 }}>
        <div style={{ position: 'absolute', top: '-10%', left: '5%', width: 700, height: 700, background: 'rgba(16,185,129,0.07)', borderRadius: '50%', filter: 'blur(120px)' }} />
        <div style={{ position: 'absolute', top: '40%', right: '-10%', width: 500, height: 500, background: 'rgba(16,185,129,0.05)', borderRadius: '50%', filter: 'blur(100px)' }} />
        <div style={{ position: 'absolute', bottom: '10%', left: '30%', width: 400, height: 400, background: 'rgba(245,158,11,0.04)', borderRadius: '50%', filter: 'blur(100px)' }} />
      </div>

      {/* ── NAVBAR ── */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(8,10,15,0.85)', backdropFilter: 'blur(20px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 12, background: 'linear-gradient(135deg, #10B981, #34D399)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(16,185,129,0.35)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button onClick={() => setAuthMode('register')} style={{ padding: '8px 20px', borderRadius: 10, background: 'linear-gradient(135deg, #10B981, #059669)', border: 'none', color: 'white', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, boxShadow: '0 4px 16px rgba(16,185,129,0.3)', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <section style={{ position: 'relative', zIndex: 1, paddingTop: 140, paddingBottom: 100, textAlign: 'center', padding: '140px 24px 100px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          {/* Badge */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', borderRadius: 999, background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)', color: '#34D399', fontSize: 12, fontWeight: 600, marginBottom: 32, letterSpacing: '0.05em' }}>
          ...
          <h1 style={{ ...S, fontSize: 'clamp(44px, 7vw, 76px)', fontWeight: 800, lineHeight: 1.08, marginBottom: 24, letterSpacing: '-0.02em' }}>
            ...
            <br />
            <span style={{ background: 'linear-gradient(135deg, #10B981, #F59E0B)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Instantly.</span>
          </h1>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 56 }}>
            <button onClick={() => setAuthMode('register')} style={{ padding: '14px 32px', borderRadius: 14, background: 'linear-gradient(135deg, #10B981, #059669)', border: 'none', color: 'white', fontSize: 15, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 8px 30px rgba(16,185,129,0.4)', fontFamily: 'DM Sans, sans-serif', transition: 'all 0.2s' }}>
        ...
        <div style={{ maxWidth: 1000, margin: '72px auto 0', position: 'relative' }}>
          <div style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.15), rgba(16,185,129,0.05))', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 24, padding: 3, boxShadow: '0 40px 100px rgba(0,0,0,0.5), 0 0 60px rgba(16,185,129,0.1)' }}>
            <div style={{ background: '#0F1218', borderRadius: 22, overflow: 'hidden' }}>
              {/* Mock browser bar */}
              <div style={{ background: '#0A0C10', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <div style={{ padding: '20px 24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <div style={{ padding: '6px 14px', borderRadius: 10, background: 'rgba(16,185,129,0.15)', border: '1px solid rgba(16,185,129,0.25)', color: '#34D399', fontSize: 12 }}>Advanced Filters</div>
                    <div style={{ padding: '6px 14px', borderRadius: 10, background: 'linear-gradient(135deg, #10B981, #059669)', color: 'white', fontSize: 12 }}>Export CSV</div>
                  </div>
                </div>
                {/* Table head */}
                ...
                {[
                  ...
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '100px 1fr 120px 90px 80px', gap: 16, padding: '10px 12px', borderRadius: 10, background: i === 1 ? 'rgba(16,185,129,0.07)' : 'transparent', borderBottom: '1px solid rgba(255,255,255,0.03)', alignItems: 'center' }}>
                    <span style={{ fontSize: 12, color: '#10B981', fontWeight: 600, fontFamily: 'monospace' }}>{row.mc}</span>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 6, background: row.entity === 'CARRIER' ? 'rgba(16,185,129,0.1)' : 'rgba(245,158,11,0.1)', color: row.entity === 'CARRIER' ? '#34D399' : '#F59E0B', border: `1px solid ${row.entity === 'CARRIER' ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)'}`, display: 'inline-block' }}>{row.entity}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Floating stat cards */}
          <div style={{ position: 'absolute', top: -16, right: -24, background: '#141820', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 16, padding: '14px 20px', boxShadow: '0 16px 40px rgba(0,0,0,0.4)', display: 'none' }} className="md:block">
            <div style={{ fontSize: 10, color: '#475569', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Email Coverage</div>
            <div style={{ ...S, fontSize: 24, fontWeight: 700, color: '#10B981' }}>68.4%</div>
          </div>
          <div style={{ position: 'absolute', bottom: 24, left: -24, background: '#141820', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 16, padding: '14px 20px', boxShadow: '0 16px 40px rgba(0,0,0,0.4)', display: 'none' }} className="md:block">
            <div style={{ fontSize: 10, color: '#475569', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Active Carriers</div>
            <div style={{ ...S, fontSize: 24, fontWeight: 700, color: '#F59E0B' }}>2.1M+</div>
          </div>
        </div>
      </section>
      ...
      <section ref={statsRef} style={{ position: 'relative', zIndex: 1, padding: '60px 24px', borderTop: '1px solid rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 0 }}>
          {[
            { value: c1 >= 4200000 ? '4.2M+' : c1.toLocaleString(), label: 'Companies Indexed', icon: Database, color: '#10B981' },
            { value: '1,000s', label: 'Data Points Per Carrier', icon: Layers, color: '#F59E0B' },
            { value: `${c3}+`, label: 'Pro Research Tools', icon: Activity, color: '#34D399' },
            { value: 'Weekly', label: 'Feature Releases', icon: Zap, color: '#FCD34D' },
          ].map((s, i) => (
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '80px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(420px, 1fr))', gap: 64, alignItems: 'center' }}>
          ...
          <div style={{ background: '#0F1218', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 24, padding: 28, boxShadow: '0 24px 60px rgba(0,0,0,0.4)' }}>
            <div style={{ marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {['State: TX', 'Entity: Carrier', 'Has Email: Yes', 'Active: Yes'].map(f => (
                  <div key={f} style={{ padding: '8px 12px', background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 10, fontSize: 12, color: '#10B981', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Filter size={11} /> {f}
                  </div>
                ))}
              </div>
      ...
      {/* ── FEATURE 2: PROFILES ── */}
      <section style={{ position: 'relative', zIndex: 1, padding: '80px 24px', background: 'rgba(15,20,25,0.5)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(420px, 1fr))', gap: 64, alignItems: 'center' }}>
          {/* Mock profile card */}
          <div style={{ background: '#0F1218', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 24, padding: 28, boxShadow: '0 24px 60px rgba(0,0,0,0.4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24, paddingBottom: 20, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ width: 52, height: 52, borderRadius: 16, background: 'linear-gradient(135deg, #10B981, #34D399)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 24px rgba(16,185,129,0.35)' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 10, padding: '2px 10px', borderRadius: 6, background: 'rgba(16,185,129,0.1)', color: '#34D399', border: '1px solid rgba(16,185,129,0.2)', fontWeight: 700 }}>CARRIER</span>
                </div>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
              {[{ l: 'DOT #', v: '3821944', c: '#10B981' }, { l: 'MC #', v: '123456', c: '#10B981' }, { l: 'Fleet Size', v: '47 units', c: 'white' }].map(s => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
              {[
                ...
                <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'rgba(255,255,255,0.02)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.04)' }}>
                  <item.icon size={14} style={{ color: '#10B981', flexShrink: 0 }} />
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              {[
                ...
                { icon: FileText, label: 'Inspection History', desc: 'Full violation records', color: '#34D399' },
                { icon: Shield, label: 'Insurance Data', desc: 'Coverage & renewal dates', color: '#F59E0B' },
                { icon: Download, label: 'CSV Export', desc: 'Bulk export ready', color: '#10B981' },
              ].map(f => (
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(420px, 1fr))', gap: 64, alignItems: 'center' }}>
          ...
          <div style={{ background: '#0F1218', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 24, padding: 28, boxShadow: '0 24px 60px rgba(0,0,0,0.4)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              <div style={{ padding: '8px 16px', borderRadius: 10, background: 'linear-gradient(135deg, #10B981, #059669)', color: 'white', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
            ...
            {[
              ...
              <div key={i} style={{ padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.04)', display: 'grid', gridTemplateColumns: '1fr 1fr 40px 80px', gap: 8, alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: '#10B981', marginTop: 2 }}>{r.email}</div>
                </div>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '80px 24px', background: 'rgba(15,20,25,0.5)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
            {[
              { icon: Database, title: 'FMCSA Database', desc: '4.2M+ carriers indexed with real-time authority status, contact info, and compliance data.', color: '#10B981', tag: 'Core' },
              ...
              { icon: ShieldCheck, title: 'Safety Intelligence', desc: 'Instant access to BASIC scores, inspection history, crash records, and OOS violation rates.', color: '#34D399', tag: 'Pro' },
              { icon: Filter, title: 'Advanced Filtering', desc: '30+ filter criteria: state, cargo type, hazmat, fleet size, years in business, and more.', color: '#10B981', tag: 'Core' },
              { icon: Download, title: 'Bulk CSV Export', desc: 'Export thousands of filtered carrier records with full contact and compliance data instantly.', color: '#059669', tag: 'Core' },
              ...
              { icon: Globe, title: 'FMCSA Register', desc: 'Monitor new authority applications and be first to reach carriers before they are established.', color: '#10B981', tag: 'Pro' },
              { icon: Users, title: 'Batch Enrichment', desc: 'Enrich thousands of carriers with safety ratings, insurance status, and contact data in bulk.', color: '#F59E0B', tag: 'Pro' },
            ].map((f, i) => (
              <div key={i} style={{ padding: 24, background: '#0F1218', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 20, position: 'relative', transition: 'all 0.2s', cursor: 'default' }}
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '80px 24px 120px' }}>
        <div style={{ maxWidth: 780, margin: '0 auto', textAlign: 'center', background: 'linear-gradient(135deg, rgba(16,185,129,0.12), rgba(16,185,129,0.06))', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 32, padding: '72px 48px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: -60, right: -60, width: 300, height: 300, background: 'rgba(16,185,129,0.12)', borderRadius: '50%', filter: 'blur(60px)', pointerEvents: 'none' }} />
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 14px', borderRadius: 999, background: 'rgba(16,185,129,0.15)', border: '1px solid rgba(16,185,129,0.3)', color: '#34D399', fontSize: 12, fontWeight: 700, marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 28 }}>
            <button onClick={() => setAuthMode('register')} style={{ padding: '16px 40px', borderRadius: 14, background: 'linear-gradient(135deg, #10B981, #059669)', border: 'none', color: 'white', fontSize: 16, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 32px rgba(16,185,129,0.4)', fontFamily: 'DM Sans, sans-serif', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <footer style={{ position: 'relative', zIndex: 1, borderTop: '1px solid rgba(255,255,255,0.05)', padding: '40px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: 10, background: 'linear-gradient(135deg, #10B981, #34D399)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {authMode && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'rgba(8,10,16,0.85)', backdropFilter: 'blur(10px)' }}>
          <div style={{ background: '#0F1218', border: '1px solid rgba(255,255,255,0.08)', width: '100%', maxWidth: 440, padding: 36, borderRadius: 24, boxShadow: '0 40px 100px rgba(0,0,0,0.6), 0 0 0 1px rgba(16,185,129,0.1)', position: 'relative' }}>
            ...
            <div style={{ marginBottom: 28 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                <div style={{ width: 36, height: 36, borderRadius: 12, background: 'linear-gradient(135deg, #10B981, #34D399)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Truck size={16} color="white" />
                </div>
                <span style={{ ...S, fontSize: 15, fontWeight: 700, color: 'white' }}>FreightIntel</span>
              </div>
            ...
            <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <button type="submit" disabled={isLoading}
                style={{ width: '100%', padding: '14px', borderRadius: 12, background: isLoading ? 'rgba(16,185,129,0.5)' : 'linear-gradient(135deg, #10B981, #059669)', border: 'none', color: 'white', fontSize: 15, fontWeight: 700, cursor: isLoading ? 'not-allowed' : 'pointer', boxShadow: '0 4px 16px rgba(16,185,129,0.3)', fontFamily: 'DM Sans, sans-serif', marginTop: 4 }}>
            ...
            <div style={{ marginTop: 20, textAlign: 'center', fontSize: 13, color: '#334155' }}>
              {authMode === 'login' ? (
                <>Don't have an account?{' '}
                  <button onClick={() => setAuthMode('register')} style={{ color: '#10B981', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Sign up free</button>
                </>
              ) : (
                <>Already have an account?{' '}
                  <button onClick={() => setAuthMode('login')} style={{ color: '#10B981', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Log in</button>
                </>
              )}
            </div>
          </div>
